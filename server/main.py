...
def update_machine_data(username):
    global consecutive_faults, machine_status_data
    print(f"{username} in data for prediction")
    if dataForPrediction and predict:
        set = 0
        while True:
            username = dataForPrediction[0]['username']
            print(f"user name for updating {username}")
            set=set+1
            ...
            for machine in Machinedata:
                ...
                print(f"Machinedata: {Machinedata}")
                print(f"Machine status data: {machine_status_data}")

                amplitude = machine['amplitudeMean']
                rms = machine['rmsMean']
                zcr = machine['zcrMean']
                ...
                machine_status_data[machine_name]['amplitude'].append(amplitude)
                machine_status_data[machine_name]['rms'].append(rms)
                machine_status_data[machine_name]['zcr'].append(zcr)
                ...
                if len(machine_status_data[machine_name]['status']) == batch_size:
                    ...
                    mean_amplitude = np.mean(machine_status_data[machine_name]['amplitude'], dtype=float)
                    mean_rms = np.mean(machine_status_data[machine_name]['rms'], dtype=float)
                    mean_zcr = np.mean(machine_status_data[machine_name]['zcr'], dtype=float)
                    buffer = True
                    ...
                    if mean_status == 1:
                        ...
                        machine['health'] = "------"
                    machine['amplitudeMean'] = mean_amplitude
                    machine['rmsMean'] = mean_rms
                    machine['zcrMean'] = mean_zcr
                    # Update the machine status based on the mean

                    update_machine.append(machine)
                    machine_status_data[machine_name] = {'status': [], 'health': [], 'amplitude': [], 'rms': [], 'zcr': []}
                    ...
                    if consecutive_faults[machine_name] >= 3:
                        ...
                        if username:
                            user_update_result = collection.update_one(
                                {'username': username},
                                {'$push': {'faults': {'fault_id':fault_id ,'machineId':machineId ,'machine name': machine_name, 'fault_time': fault_time}}}
                            )
            ...
            if buffer:
                ...
                if username:
                    ...
                    if data_update_result.modified_count > 0:
                        print(f"MACHINE DATA UPDATED {username}")
                update_machine = []
...
def get_plant():
    ...
    username=data.get('username')
    print(f"user name from plant data {username}")
    ...
    try:
        ...
        with thread_lock:
            if thread is None or not thread.is_alive():
                print(f"user name from parameter of data prediction {username}")
                thread = socketio.start_background_task(update_machine_data,username)
...
def get_plant_details():
    try:
        username=request.args.get('username')
        print(f"{username} in get_plant_details")
        if username:
...
@app.route('/get_plot_image/<filename>', methods=['GET'])
...
def machine_analytics(machineId):
    try:
        ...
        user = collection.find_one({"username": username}, {"machinedata": 1, "faults" : 1})
        ...
        print(machineId)
        machine_record = []
        for batch in all_machine_records:
            if int(batch['machineId']) == int(machineId):
                machineName = batch['machine_name']
                machine_record.append(batch)
        ...
        healthy_count = sum(1 for record in machine_record if record['health'] == "HEALTHY")
        faulty_count = sum(1 for record in machine_record if record['health'] == "FAULTY")
        nil_count = sum(1 for record in machine_record if record['health'] == "------")
        ...
        if total_records > 0:
            ...
            healthy_percentage=(healthy_count/total_records)*100
            faulty_percentage=(faulty_count/total_records)*100
            nil_percentage=(nil_count/total_records)*100
            for machine in machine_record:
                if machine['status'] == 'RUNNING':
                    ...
                    status.append(0)
                time = machine['time']
                amplitude = float(machine['amplitudeMean'])
                rms= float(machine['rmsMean'])
                zcr = machine['zcrMean']
                timestamps.append(time)
                amplitudemean.append(amplitude)
                rmsmean.append(rms)
                zcrmean.append(zcr)
        else:
            running_percentage = 0
            not_running_percentage = 0
            nil_percentage=healthy_percentage=faulty_percentage=0

        if 'faults' in user:
            faults = [fault for fault in user['faults'] if int(fault['machineId']) == int(machineId)]
            print(f"Faults data:{faults}")

            fault_times = [datetime.strptime(fault['fault_time'], '%Y-%m-%d %H:%M:%S') for fault in faults]

            if time_filter:
                if time_filter == 'day':
                    fault_groups = [fault_time.strftime('%Y-%m-%d') for fault_time in fault_times]
                elif time_filter == 'month':
                    fault_groups = [fault_time.strftime('%Y-%m') for fault_time in fault_times]
                elif time_filter == 'year':
                    fault_groups = [fault_time.strftime('%Y') for fault_time in fault_times]
                else:
                    return jsonify({"message": "Invalid time filter"}), 400
            else:
                return jsonify({"message": "Time filter not provided"}), 400  # Handle the absence of a time filter

            fault_counts = dict(Counter(fault_groups))
        else:
            fault_counts = {}

        faulty_plot_image_url = f"http://localhost:5007/get_plot_image/FAULTY{machineId}.png"
        healthy_plot_image_url = f"http://localhost:5007/get_plot_image/HEALTHY{machineId}.png"
        notrunning_plot_image_url = f"http://localhost:5007/get_plot_image/NOTRUNNING{machineId}.png"
        ...
        summary_data = {
            ...
            'amplitude_mean' : amplitudemean,
            'rms_mean' : rmsmean,
            'zcr_mean' : zcrmean,
            'status' : status,
            'healthy_plot': healthy_plot_image_url,
            'faulty_plot': faulty_plot_image_url,
            'notrunning_plot': notrunning_plot_image_url,
        }
